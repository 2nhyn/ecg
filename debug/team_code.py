...
def train_model(data_folder, model_folder, verbose=False):
    """
    대회 측이 호출하는 유일한 함수. data_folder에서 직접 로드 → 3단계 파이프라인 → 모델 저장.
    """
    ...
    # ==================== 여기를 수정하세요! ====================
    # 모든 신호의 길이를 4096으로 통일하기 위해 패딩을 추가합니다.
    # run_model 함수에서 사용하는 것과 동일한 MAX_LEN을 사용합니다.
    MAX_LEN = 4096
    padded_signals_array = pad_signals(all_signals, MAX_LEN)

    # ContrastiveECGDataset은 리스트 형태의 입력을 기대하므로,
    # 패딩된 numpy 배열을 다시 파이썬 리스트로 변환해줍니다.
    all_signals_padded = [padded_signals_array[i] for i in range(len(padded_signals_array))]
    # ==========================================================

    # 패딩된 데이터를 학습 함수에 전달합니다.
    ...
    best_ckpt = run_phase2(
        ...
        lin_epochs=5, ft_epochs_base=3, batch_size=128, lr=1e-3, wd=1e-4,
        focal_gamma=None, amp=True, num_workers=2
    )
...
def load_model(model_folder: str, verbose: bool = False):
    ...
    ckpt_p1 = os.path.join(model_folder, "phase1_encoder.pt")
