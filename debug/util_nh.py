...
def load_all_sources(data_folder: str, verbose: bool = False):
    """
    helper_code 규약으로 data_folder에서 ECG를 읽어
    (PTB-XL, SaMi-Trop, CODE-15)로 분리하여 반환.
    반환:
      (ptb_sig, ptb_lab), (sami_sig, sami_lab), (code_sig, code_lab, code_conf)
    * 각 신호는 (T,12) float32, 전처리(bandpass) + MAD 정규화 적용
    * code_conf는 파일에 없으므로 1.0으로 채움
    """
...
def run_phase1_contrastive(all_signals, save_dir, epochs=100, 
                           ...
                           wd=1e-4, amp=True, num_workers=6):
    wandb.init(
    ...
    for ep in range(1, epochs+1):
        ...
        wandb.log({"phase1_loss": run_loss/len(loader), "epoch": ep})
        print(f"[P1] epoch {ep}/{epochs} loss={run_loss/len(loader):.4f}")
...
def run_phase2(
    ...
    # loaders/opt
    batch_size=64, lr=1e-4, wd=1e-4, focal_gamma=None, amp=True, num_workers=6
):
    """
    Phase 2:
      1) Linear probe → 2) Warm-up fine-tune(PTB+SaMi 중심, CODE15 소량 랜덤) →
      3) CODE-15 신뢰도 산출 & 라벨 일치 필터 → 4) 추가 fine-tune (선택 서브셋 가중)
    """
    wandb.init(
    ...
    neg_loader = DataLoader(neg_ds, batch_size=batch_size, shuffle=False, num_workers=num_workers, pin_memory=True)
    pos_loader = DataLoader(pos_ds, batch_size=batch_size, shuffle=False, num_workers=num_workers, pin_memory=True)
    ...
    for ep in range(1, ft_epochs_base+1):
        ...
        wandb.log({
            "warmup_loss": tr_loss/len(train_loader),
            "val_acc": acc,
            "epoch": ep
        })
        print(f"[Phase2][warmup {ep}/{ft_epochs_base}] loss={tr_loss/len(train_loader):.4f} | val_acc={acc:.4f}")
        if acc > best_acc:
            best_acc = acc
    ...
    code_loader = DataLoader(code_ds, batch_size=batch_size, shuffle=False, num_workers=num_workers, pin_memory=True)
    ...
    with torch.no_grad():
        for xb, yb in code_loader:
            ...
            for i, keep in enumerate(keep_mask.tolist()):
                if keep:
                    x_i = xb[i].detach().cpu().numpy().T.copy()  # (T,C), own storage
                    kept_signals.append(x_i)      # (C,T) tensor
                    kept_labels.append(int(yb_cpu[i].item()))
                    kept_weights.append(float(s[i].item()))  # s를 가중치로
    ...
    ext_signals = list(ptb_sig) + list(sami_sig) + kept_signals
    ...
    for ep in range(1, ft_epochs_pl+1):
        ...
        acc = evaluate_acc(encoder, heads, val_loader, device)
        wandb.log({
            "pl_loss": tr_loss/len(ext_loader),
            "val_acc": acc,
            "epoch": ep
        })
        print(f"[Phase2][PL {ep}/{ft_epochs_pl}] loss={tr_loss/len(ext_loader):.4f} | val_acc={acc:.4f}")
        if acc > best_acc:
            best_acc = acc
            torch.save({"encoder": encoder.state_dict(), "heads": heads.state_dict()}, best_path)
            print(f"  ↳ saved: {best_path} (acc={best_acc:.4f})")
